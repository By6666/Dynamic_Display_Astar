...
inline void Astar::NeiborsPush_26_5(std::vector<CellPosState>& neighbors,
                                    ...
                                    T (*second)(const T&)) {
  T temp = first(pos);
  if (IsInMapBoundry(temp) && !IsObstacle(temp) && !IsObstacle(second(pos))) {
    neighbors.push_back(
        CellPosState(CodeID(temp, col_), calculate_arct2(pos, temp)));
  }
}
...
inline bool Astar::NeiborsPush(std::vector<CellPosState>& neighbors,
  ...
  if (IsInMapBoundry(temp) && !IsObstacle(temp)) {
    neighbors.push_back(CellPosState(CodeID(temp, col_), yaw));
    return true;
  } else {
    return false;
  }
...
std::vector<CellPosState> Astar::GetNeighbor(const CellPosState& pos) {
  ...
  if (curr_pos == start_pos_) {
    ...
    if (IsInMapBoundry(neig) && !IsObstacle(neig)) {
      neighbors.push_back(CellPosState(CodeID(neig, col_), yaw));
    }
  }
  ...
  while (!state_flg) {
    ...
    if (state_flg == 0 && TwoDoubleEqual(yaw, 0.0)) {
      ...
      if (!NeiborsPush(neighbors, curr_pos, ExpandPos::Dw, 0.0)) break;
    ...
    if (state_flg == 0 && TwoDoubleEqual(yaw, M_PI_2)) {
      ...
      if (!NeiborsPush(neighbors, curr_pos, ExpandPos::Rg, M_PI_2)) break;
    ...
    if (state_flg == 0 && TwoDoubleEqual(yaw, -M_PI_2)) {
      ...
      if (!NeiborsPush(neighbors, curr_pos, ExpandPos::Lf, -M_PI_2)) break;
    ...
    if (state_flg == 0 &&
      ...
      if (!NeiborsPush(neighbors, curr_pos, ExpandPos::Up, M_PI)) break;
    ...
    if (state_flg == 0 &&
      ...
      NeiborsPush(neighbors, curr_pos, ExpandPos::Dw, 0.0);
      NeiborsPush_26_5(neighbors, curr_pos, ExpandPos::Dw_Rg_26_5,
                       ExpandPos::Dw_Rg);
      break;
    }
    ...
    if (state_flg == 0 &&
      ...
      NeiborsPush(neighbors, curr_pos, ExpandPos::Rg, M_PI_2);
      NeiborsPush_26_5(neighbors, curr_pos, ExpandPos::Rg_Up_26_5,
                       ExpandPos::Up_Rg);
      break;
    }
    ...
    if (state_flg == 0 &&
      ...
      NeiborsPush(neighbors, curr_pos, ExpandPos::Lf, -M_PI_2);
      NeiborsPush_26_5(neighbors, curr_pos, ExpandPos::Lf_Dw_26_5,
                       ExpandPos::Dw_Lf);
      break;
    }
    ...
    if (state_flg == 0 &&
      ...
      NeiborsPush(neighbors, curr_pos, ExpandPos::Up, M_PI);
      NeiborsPush_26_5(neighbors, curr_pos, ExpandPos::Up_Rg_26_5,
                       ExpandPos::Up_Rg);
      break;
    }
  ...
  for (auto& elem : neighbors) {
    if (CollisionChecking(col_, curr_pos, DecodeID(elem.id, col_), elem.yaw,
                          obstacle_list_)) {
      continue;
    } else {
